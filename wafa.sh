...
echo -e "${BLUE}"
echo "=================================================="
echo "           Pterodactyl Protection - Developer By Wafa"
echo "           Created by WAFA"
echo "=================================================="
echo -e "${NC}"
    ...
    public function handle(Server $server): void
    {
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('Akses ditolak: Anda hanya dapat menghapus server milik Anda sendiri. Protection By Wafacraft - Wafa Tech');
                }
            }
...
class UserController extends Controller
{
    ...
    public function delete(Request $request, User $user): RedirectResponse
    {
        ...
        if ($request->user()->id !== 1) {
            throw new DisplayException("❌ Hanya admin ID 1 yang dapat menghapus user lain!. Protection by Wafa - Wafa Tech V1.2");
        }
        // ============================================
    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        foreach ($restrictedFields as $field) {
            if ($request->filled($field) && $request->user()->id !== 1) {
                throw new DisplayException("⚠️ Data hanya bisa diubah oleh admin ID 1. Protection by Wafacraft - Wafa Tech V1.2");
            }
        }

        // Cegah turunkan level admin ke user biasa
        if ($user->root_admin && $request->user()->id !== 1) {
            throw new DisplayException("🚫 Tidak dapat menurunkan hak admin pengguna ini. Hanya ID 1 yang memiliki izin. Protection By WafaCoolcraft - Wafa Tech V1.2");
        }
        // ====================================================
...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang dapat membuka menu Nodes. Protect by Wafacraft - Wafa Tech V1.2');
        }
        // ======================================================
...
class NestController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin utama (ID 1) yang bisa membuka menu Nests. Protect by WafaCraft - Wafa Tech V1.0');
        }
